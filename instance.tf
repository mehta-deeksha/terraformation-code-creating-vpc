...
resource "aws_instance" "jenkins-master" {
  ...
  provisioner "local-exec" {
    command = <<EOF
aws --profile ${var.profile} ec2 wait instance-status-ok --region ${var.egion-jenkins-master} --instance-ids ${self.id}
...
resource "aws_instance" "jenkins-worker-oregon" {
  ...
  subnet_id                   = aws_subnet.subnet-worker_oregon.id
  ...
  provisioner "local-exec" {
    command = <<EOF
aws --profile ${var.profile} ec2 wait instance-status-ok --region ${var.region-jenkins-worker} --instance-ids ${self.id}
